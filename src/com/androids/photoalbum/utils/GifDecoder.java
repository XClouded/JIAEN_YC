...
public class GifDecoder {
	...
	protected RectF lastRect; // last image rect
			...
			if (lastImage != null) {
				...
				if (lastDispose == 2) {
					...
					if (transparency) {
						...
						image.eraseColor(lastBgColor); // use given background
														// color
					}
		...
		for (int i = 0; i < ih; i++) {
			...
			if (line < height) {
				...
				while (dx < dlim) {
					...
					dx++;
					if(sx == 3701){
						Log.d("GifDecoder", "index is " + index + ",c" + c + ",dx" + dx);
					}
				}
			}
		}
	...
	protected void resetFrame() {
		lastDispose = dispose;
		lastRect = new RectF(ix, iy, iw, ih);
		lastImage = image;
		lastBgColor = bgColor;
		dispose = 0;
		transparency = false;
