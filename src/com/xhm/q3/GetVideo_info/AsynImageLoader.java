...
public class AsynImageLoader {
	...
	public Bitmap loadImageAsyn(String path, ImageCallback callback) {
		...
		if (caches.containsKey(path)) {
			...
			// 如果缓存中未得到缓存的图片，则从文件中读取该图片
			// 获取本程序的 图片类文件存储路径
			File dir = mContext
					.getExternalFilesDir(Environment.DIRECTORY_PICTURES);

			// 创建要获取的图片路径
			File file = new File(dir, path.replace(
					"http://www.uuunm.com/video/", ""));

			// 从文件中读取指定路径的图片
			bitmap = BitmapTools.getbiBitmap(file.getAbsolutePath());
			// 如果文件中存在该图片，则返回读取到的图片对象
		...
		public void run() {
			while (isRunning) {
				...
				while (taskQueue.size() > 0) {
					...
					File file = new File(dir, task.path.replace(
							"http://www.uuunm.com/video/", ""));
					try {
